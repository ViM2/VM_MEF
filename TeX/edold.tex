...
L'\textcolorblue{ordre} d'une ED est le degré non nul le plus haut des $a_i$, ici $n$.
...
Si on note $B$ une primitive de la fonction $\dfrac{c(x) e^A}{a(x)}$, l'ensemble des solutions est alors:
\begin{equation}
    k(x) = B(x) + C
\end{equation}
...
On cherche des solutions sous forme exponentielle $f(x) = e^{\lambda x}$. 
...
les solutions de l'ED sont \textcolorred{engendrées} par $f_1(x) = e^{\lambda_1x}$ et
...
\textcolorred{engendrées} par $f_1(x) = e^{\lambda_1x}$ et
$f_2(x) = e^{\lambda_2x}$, i.e. de la forme:
\begin{equation}
f(x) = C_1f_1(x) + C_2f_2(x) 
\end{equation}
...
On peut agir de la même manière que pour les ED d'ordre un, et les mêmes
...
La \textcolorblue{méthode de Newmark (1959)} permet de résoudre numériquement des ED du 
second ordre. 

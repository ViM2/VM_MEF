...
\medskip
L'\emph{interpolation} est une opération consistant à approcher une courbe qui n'est connue que par la donnée d'un nombre fini de points (ou une fonction à partir de la donnée d'un nombre fini de valeurs). 
...
\begin{histoire}
Le jour du Nouvel An de 1801, l'astronome italien Giuseppe Piazzi\index[aut]{Piazzi (Giuseppe), 1746-1826, Italien} a découvert l'astéroïde Cérès; il a suivi sa trajectoire jusqu'au 14 février 18012. Durant cette année, plusieurs scientifiques ont tenté de prédire sa trajectoire sur la base des observations de Piazzi mais à cette époque, la résolution des équations non linéaires de Kepler\index[aut]{Kepler (Johannes), 1571-1630, Allemand} de la cinématique est un problème très difficile. La plupart des prédictions sont erronées et le seul calcul suffisamment précis pour permettre au baron Franz Xaver von Zach\index[aut]{Zach (Franz Xaver, Baron von -), 1754-1832, Allemand} de localiser à nouveau Cérès à la fin de l'année est celui de Gauß,\index[aut]{Gauß (Johann Carl Friedrich), 1777-1855, Allemand} (alors âgé de 24 ans). Gauß avait déjà réalisé l'élaboration des concepts fondamentaux en 1795, lorsqu'il avait 18 ans. Cependant, sa méthode des moindres carrés ne fut publiée qu'en 1809 dans le tome 2 de ses travaux sur la Mécanique céleste \emph{Theoria Motus Corporum Coelestium in sectionibus conicis solem ambientium}. Le mathématicien français Adrien-Marie Legendre\index[aut]{Legendre (Adrien-Marie), 1752-1833, Français} a développé indépendamment la même méthode en 1805. Le mathématicien américain Robert Adrain\index[aut]{Adrain (Robert), 1775-1843, Américain} a publié en 1808 une formulation de la méthode. 

En 1829, Gauß a pu donner les raisons de l'efficacité de cette méthode: celle-ci est optimale à l'égard de bien des critères. Cet argument est maintenant connu sous le nom de \textcolorblue{théorème de Gauß-Markov}\index[aut]{Markov (Andreï Andreïevitch), 1856-1922, Russe}. Ce théorème dit que dans un modèle linéaire dans lequel les erreurs ont une espérance nulle, sont non corrélées et dont les variances sont égales, le meilleur estimateur linéaire non biaisé des coefficients est l'estimateur des moindres carrés. Plus généralement, le meilleur estimateur linéaire non biaisé d'une combinaison linéaire des coefficients est son estimateur par les moindres carrés. On ne suppose pas que les erreurs possèdent une loi normale, ni qu'elles sont indépendantes mais seulement non corrélées, ni qu'elles possèdent la même loi de probabilité.\\[5pt]
\footnotesize
\setlength\tabcolsep{0pt}
\begin{tabular}{ccccc} 
\includegraphics[height=23mm]{Piazzi}& 
\includegraphics[height=23mm]{Gauss}& 
\includegraphics[height=23mm]{Legendre}& 
\includegraphics[height=23mm]{Adrain}& 
\includegraphics[height=23mm]{Markov}\\ 
Piazzi & Gauß& Legendre & Adrain& Markov
...
\begin{equation}
 L_n(x)=\frac{e^x}{n!}\frac{d^n}{dx^n}\left(\mathrm{e}^{-x} x^n\right). 
\end{equation}
...
%%%%----------------------------------------------------------
Les \emph{polynômes de Bernstein} permettent de donner une démonstration constructive du théorème de Stone-Weierstrass. Dans le cadre de ce cours, nous les présentons surtout car ils sont utilisés dans la formulation générale des courbes de Bézier.\index[aut]{Bézier (Pierre), 1910-1999, Français} Pour un degré $n$, il y a $n+1$ polynômes de Bernstein $B^n_0, \dots, B^n_n$ définis, sur l'intervalle $[0,1]$ par:\index{polynôme! de Bernstein} 
\begin{equation}
 B_i^n(u) = 
\begin{pmatrix} n \\ i 
...
\begin{equation}
\dsum_{i=0}^n B_i^n(u) = 1,\qquad \forall u \in [0,1]
\end{equation}
\item Positivité:
\begin{equation}
B_i^n(u)\geq 0,\qquad\forall u\in [0,1],\quad\forall i\in 0,\dots,n
\end{equation}
\item Symétrie:
\begin{equation}
B_i^n(u) = B_{n-i}^n(1-u), \qquad \forall u \in [0,1],\quad \forall i \in 0, \dots, n
\end{equation}
...
\end{cases},\qquad \forall u \in [0,1]
\end{equation}
\end{itemize}
...
\begin{equation}
b(t)=\dsum_{i=0}^n B_i^n(t)\cdot\mathbf{P}_i\quad\text{avec}\quad t \in[0,1]
\end{equation}
...
\item la courbe commence par le point $\mathbf{P}_0$ et se termine par le point $\mathbf{P}_n$, mais ne passe pas à priori par les autres points de contrôle;
...
\subsection{B-Spline}
%%%%----------------------------------------------------------
Une \emph{B-spline} est une combinaison linéaire de splines positives à support compact minimal. Les B-splines sont la généralisation des courbes de Bézier, elles peuvent être à leur tour généralisées par les NURBS.\index[aut]{Bézier (Pierre), 1910-1999, Français} Étant donné $m+1$ points $t_i$ dans $[0,1]$ tels que $0\le t_0\le t_1\le\ldots\le t_m\le 1$, une courbe spline de degré $n$ est une courbe paramétrique $\mathbf{S}:[0,1]\to\mathbb{R}^d$, composée de fonctions \emph{B-splines} de degré $n$: 
\begin{equation}

\chapter{Le calcul efficient: qualité des résultats et efforts de calcul}\label{Ch-Amelio}
...
solution obtenue:
\begin{itemize}
   \item la \textcolorblue{méthode -r-}\index{Méthode r} pour \textcolorblue{relocalisation des nœuds}.
	...
	elle est limitée.
   \item la \textcolorblue{méthode -h-}\index{Méthode h} où $h$ est la taille d'un élément.
   ...
   \item la \textcolorblue{méthode -p-}\index{Méthode p} pour \textcolorblue{polynômes}.
...
\medskip
\section{Le Post-traitement}\index{Post-traitement}\label{Sec-PT}
...
\medskip
\section{La sous-structuration et la simulation multi-échelles}\label{Sec-ssstruc}\index{Sous-structuration}\index{Multi-échelles}
...
\begin{figure}[htb]
\centerline{\includegraphics[height=60mm]{avion.eps}}
...
\begin{itemize}
   ...
   \item \textcolorblue{Un modèle microcopique}\index{Modèle microscopique}
...
\begin{itemize}
   ...
   \item Le modèle de comportement microcopique:\index{Modèle microscopique}
...
\begin{itemize}
   \item \textcolorblue{Condition de périodicité}\index{Condition de périodicité}
	...
	On postule que l'on connaît l'allure du reste. On peut alors enchainer un calcul macro
	avec un solveur spécifique pui un calcul micro avec un solveur classique;
...
recourir à des calculatreur parallèles.}
...
Pour cette raison, elle fait toujours l'ojet de nombreux projets de recherche. 
...
\medskip
\section{Les Super-Élements}\index{Super-élément}
...
d'un sensemble d'éléments. On parle alors de macro-élément.\index{Macro-élément}
...
Mézalor, quand parle-t-on de sous-structure ou de macro-élément?\index{Macro-élément}\index{Sous-structure}\index{Sous-structuration}
...
problèmes \og relativement proches\fg{} les uns des autres (i.e. tester plusieurs configurations).
...
en fonction des autres matrices. On rappelle que $\MM{\Delta K}$ est appelé \textcolorblue{perturbation}
...
Pour cela, on s'appuie sur la connaisance de la matrice $\MM{K}$: celle-ci est singulière d'ordre
...
de la valeur propore nulle dont $R$ est une base, mais \textcolorred{elle ne possède pas
de caractère bande}.
...
Nous n'entrons pas dans le détail, mais en cours de calcul se pose la question intéressante:
...
\medskip
\textcolorgreen{Encore une fois, nous n'avons fait qu'effleurer le problème, juste pour \og faire 
connaître \fg{} la méthode. Il nous semblait intéressant de présenter la notion de dérivée

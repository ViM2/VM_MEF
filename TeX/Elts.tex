...
On dit que $\Sigma$ est \textcolorblue{$P$-unisolvant} ssi pour tous réels $\alpha_1$, ..., $\alpha_N$, 
...
\medskip
\subsection{EF de Lagrange}\index{EF de Lagrange}
...
\medskip
\begin{definition}[famille affine d'EF]
On appelle \textcolorblue{famille affine d'éléments finis} une famille d'EF
...
\begin{itemize}
   \item $K$ est un élément géométrique de $\RR^n$, compact, connexe, et d'intérieur
	non vide;
   \item $\Sigma=\{\sigma_1, ..., \sigma_N\}$ un ensemble de $N$ \textcolorred{formes linéaires}
...
\medskip
Les \textcolorblue{fonctions de base globales} $\varphi_i$, $(i=1, ..., N_h)$ sont 
définies par :
\begin{equation}

...
\medskip
Pour palier ces problèmes ont été développées, dès les années 70 puis améliorées et popularisées dans les années 90, de nombreuses méthodes où la discrétisation ne repose plus sur un maillage mais sur un ensemble de points\footnote{On peut par exemple nommer la méthode des éléments diffus, la méthode de Galerkin sans éléments (Element Free Galerkin Method, EFGM), Reproducing Kernel Particle Method (RKPM), $h-p$ Cloud Method...}.
L'idée est de construire une fonction définie sur un espace continu à partir de l'ensemble des valeurs discrètes prises par cette fonction sur un nuage de points du domaine. On parle alors de \textcolorblue{méthode particulaire}. Les problèmes de déformation de maillage ne peuvent survenir puisqu'il n'y a plus de connectivité entre les nœuds; le remaillage et la gestion des frontières internes (fissures...) sont résolus puisqu'il s'agit alors simplement d'ajouter des nœuds au grès des besoins.

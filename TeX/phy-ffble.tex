...
\colorgreen
\noindent\textbf{Rappel méthodologique:} La formulation variationnelle d'un problème s'obtient en multipliant l'équation d'état du problème par une fonction test, en intégrant par parties, et en utilisant les conditions aux limites. (De plus, en mécanique, la formulation variationnelle est le principe des puissances virtuelles).
\colorblack
...
\medskip
\begin{remarque}[Une autre formulation variationnelle]\colorgris
...
On utilise ensuite la surjectivité de l'application trace, $\gamma_0$ (ce qui devrait malgré tout rester compréhensible bien que la trace ait été définie de manière «un peu légère» au paragraphe~\ref{Sec-SoboTrace}).
...
\medskip
Il y aurait lieu à quelques discussions sur l'interprétation des résultats, mais celle-ci serait beaucoup plus abstraite que dans le cas du Laplacien de Dirichlet. Le lecteur désireux de rentrer (à raison) dans ce genre de détails trouvera aisément un cours de M2 sur le sujet. Cela dépasse le cadre que nous nous sommes fixé dans ce document.
...
\medskip
\subsubsection{Laplacien avec conditions mixtes en formulation mixte}\index{ED-EDP!de Laplace}\index{laplacien}\index{ED-EDP!de Poisson}\index{condition aux limites!mêlée}\index[aut]{Laplace (Pierre Simon de -), 1749-1827, Français}\index[aut]{Poisson (Siméon Denis), 1781-1840, Français}\index[aut]{Neumann (Carl Gottfried), 1832-1925, Allemand}\index{condition aux limites!de Neumann}
...
\right. 
\end{equation}
avec~$\XX=\{v\in H(\dive,\Omega); v.n=0 \text{ sur } \Gamma_N\}$.
...
que pour presque tout~$t$, $v(t,\cdot)$ est une fonction de~$H^1_0(\Omega)$ et $\int_0^T \|v\|^2_{H^1_0(\Omega)}<\infty$. Le premier est un espace de Banach, le second, un espace de Hilbert.
...
\medskip
Si en plus le matériau est isotrope, cette expression se simplifie ($\lambda$ et~$\mu$ sont les coefficients de Lamé) en:
...
\medskip
\subsection{Équation des plaques}

Soit~$\Omega$ un ouvert borné régulier de~$\RR^n$, l'équation des plaques est:
\begin{equation}\label{Eq-Plaques}
...
\left\{\begin{array}{ll}
u=0 & \text{ sur } \Gamma=\partial\Omega\\
\dfrac{\partial u}{\partial n} = 0 & \text{ sur } \Gamma
\end{array}\right.
\end{equation}

La formulation variationnelle correspondante est:
\begin{equation}\label{Eq-Plaques-var}
\dint_\Omega \Delta u\Delta v = \dint_\Omega fv, \qquad \forall v\in V
\end{equation}
où~$V$ est l'espace des fonction~$C^2(\overline{\Omega})$ vérifiant les conditions aux limites, i.e. telles que~$v=0$ et $\partial v/\partial n = 0$ sur~$\Gamma$ (démonstration 1 ci-dessous), ou~$V$ est l'espace~$H^2_0(\Omega)$ (démonstration 2 ci-dessous).

\begin{demonstration}\footnotesize\colorgris
Nous allons faire l'effort ici d'écrire la démonstration.

À priori, une solution~$u$ de l'équation des plaques est une fonction de~$C^4(\overline{\Omega})$.

Soit~$u$ une solution régulière de l'équation des plaques~(\ref{Eq-Plaques}). Pour toute fonction$v\in V$, on a:
\[ \dint_\Omega \Delta(\Delta u) v = \dint_\Omega fv \]
Par intégration par parties, il vient:
\[  \dint_\Omega \Delta(\Delta u) v = -\dint_\Omega \nabla (\Delta u).\nabla v +
\dint_\Gamma \dfrac{\partial(\Delta u}{\partial n} v \]
or~$v=0$ sur~$\Gamma$, d'où:
\[  \dint_\Omega \Delta(\Delta u) v = -\dint_\Omega \nabla (\Delta u).\nabla v \]
Une seconde intégration par parties conduit à:
\[  \dint_\Omega \Delta(\Delta u) v = \dint_\Omega \Delta u\Delta v -
\dint_\Gamma \Delta u \dfrac{\partial v}{\partial n} \]
et comme~$\partial v/\partial n=0$, il reste:
\[  \dint_\Omega \Delta(\Delta u) v = \dint_\Omega \Delta u\Delta v \]
...
Supposons donc maintenant que~$u$ est une solution du problème~(\ref{Eq-Plaques-var}).
En effectuant deux intégration par parties successives, il vient:
\[ \dint_\Omega \left( \Delta(\Delta u) -f\right) v =0 \]
pour toute fonction~$v\in V$.
Or~$V$ est un sous-espace dense de~$L^2(\Omega)$, et donc:
\[ \Delta(\Delta u) - f =0 \]~$\square$\colorblack
\end{demonstration}

\begin{demonstration}[Autre démonstration par application du théorème de Lax-Milgram]\footnotesize\colorgris
La formulation variationnelle associée à l'équation des plaques consiste à déterminer~$u\in H^2_0(\Omega)$ tel que:
\[ a(u,v) = b(v) \qquad \forall v\in H^2_0(\Omega)\]
avec:
\[ a(u,v)= \dint_\Omega \Delta u \Delta v \quad \text{ et } \quad
b(v) = \dint_\Omega fv \]

La seule hypothèse non triviale à vérifier afin de pouvoir appliquer le théorème de Lax-Milgram est la coercitivité de la forme bilinéaire~$a(.,.)$.
Pour tout~$u\in H^2_0(\Omega)$, deux intégrations par parties successives conduisent à:
\[ a(u,u) = \dsum_{i,j} \dint_\Omega \left| \dfrac{\partial^2 u}{\partial x_i\partial x_j}\right|^2 = \|\nabla^2 u\|_{L^2(\Omega)} \]
L'application de l'inégalité de Poincaré deux fois permet de conclure à l'existence de deux constantes positives~$C$ et~$C'$ telles que, $\forall u\in H^2_0(\Omega)$ on ait:
\[ \|u\|^2_{L^2(\Omega)} \le C \|\nabla u\|^2_{L^2(\Omega)} \le 
C' \|\nabla^2 u\|_{L^2(\Omega)} = C' a(u,u) \]
et donc, il existe une constante positive~$C$ telle que $\forall u\in H^1_0(\Omega)$:
\[ \|u\|^2_{H^2(\Omega)} \le C a(u,u) \]

La coercitivité de~$a(.,.)$ est établie, et le théorème de Lax-Milgram nous permet de conclure à l'existence et à l'unicité de la solution.

\medskip
Vérifions maintenant que, réciproquement, la solution du problème variationnel est solution du problème avec conditions aux limites.

Pour cela, considérons~$\omega$ un ouvert inclus dans le compact~$\Omega$. Il existe une fonction~$\chi\in C_c^\infty(\Omega)$ telle que~$\chi=1$ sur~$\omega$. Pour toute fonction~$v\in C_c^\infty(\Omega)$ de support inclus dans~$\omega$:
\[ \dint_\Omega \chi \Delta u\Delta v = \dint_\Omega \Delta u\Delta v = \dint_\Omega fv \]

Or on sait (enfin dira-ton, sinon on admettra) que:
si~$w$ et~$f$ sont des fonctions de~$L^2(\Omega)$ vérifiant pour toute fonction~$v\in C_c^\infty(\Omega)$:
\[ -\dint_\Omega w\Delta v =\dint_\Omega fv \]
alors~$(\chi w)\in H^2(\Omega)$ quelle que soit la fonction~$\chi\in C_c^\infty(\Omega)$.

D'où, l'on déduit que~$\chi\Delta u\in H^2(\Omega)$. Il est par conséquent licite d'effectuer deux intégrations par parties successives, et l'on obtient:
\[ \dint_\Omega \Delta(\chi\Delta u) v = \dint_\Omega fv \]
pour toute fonction~$v\in C_c^\infty(\Omega)$ de support inclus dans~$\omega$. On en déduit que pour presque tout~$x\in\omega$:
\[ \Delta(\Delta u)(x) = f(x) \]

En considérant une suite~$\omega_n$ de compacts dont l'union est~$\Omega$, on en déduit que cette relation reste valable pour presque tout~$x\in\Omega$.

Enfin, comme~$u\in H^2_0(\Omega)$, la solution du problème variationnel vérifie automatiquement les conditions aux limites.~$\square$\colorblack
\end{demonstration}
...
Soit~$\Gamma=\partial\Omega$ la frontière du domaine concerné. On considère que~$\Gamma$ est \textcolorblue{partitionnée} en trois zones (i.e. dont l'union vaut~$\Gamma$ et dont aucune n'intersecte les autres) qui portent chacune une condition aux limites différente:~$\Gamma_D$ de Dirichlet, $\Gamma_N$ de Neumann et~$\Gamma_R$ de Robin.
\medskipvm
On a donc:
\begin{itemize}
...
$\overline{V}_n$ est la valeur imposée de la composante normale de la vitesse. Cette condition limite modélise la \textcolorred{vibration} d'un panneau, ce qui en fait la condition la plus souvent utilisée lorsque l'on a un couplage faible entre la structure et le fluide. On effectue alors une étude vibratoire de la structure, puis grâce à celle-ci une étude acoustique du fluide sans se préoccuper des interactions fluide-structure.

\noindent\textbf{Condition de Robin:}
$A_n$ est appelé coefficient d'admittance, il vaut l'inverse de l'impédance~$Z_n$:
$A_n\cdot Z_n=1$. Physiquement, ce coefficient représente l'\textcolorred{absorption} et la \textcolorred{reflexion} de l'onde par le matériau environnant dont la frontière est modélisée par~$\Gamma_R$.
\colorblack
\end{remarque}
...
\medskip
\textcolorgreen{En fonction des conditions aux limites et des conditions initiales du problème, la variation de pression solution du problème peut être réelle, i.e. être de la forme~$p'(x,y,z,t)=\Re(p(x,y,z)e^{i\omega t})$. Une telle solution est appelée \textcolorblue{monochromatique}. On pourra alors résoudre le problème en restant dans~$\RR$ au lieu de passer dans~$\CC$.
Cela se produit lorsque l'on résout l'équation d'Helmholtz~(\ref{Eq-Helm}) assortie uniquement de conditions aux limites de Dirichlet.}

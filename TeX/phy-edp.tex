...
leurs dérivées. 
\textcolorblue{L'ordre d'une ED} correspond au degré maximal de dérivation auquel l'une des fonctions inconnues 
a été soumise.
...
\medskip
\begin{histoire}%
Les problèmes posés ou menant à des ED sont aussi vieux que l'analyse elle-même (XVIIe-XVIIIe,
...
\bigskip
\ImageADroite{%
On résoud les ED par la méthode des séries entières sans s'encombrer des notions
...
des ED.
La liste des ED que l'on sait résoudre de cette manière est plutôt maigre:
...
les ED linéaires du premier ordre, puis Riccati...\index[aut]{Riccati (Jacopo Francesco), 1676-1754, Italien}%
}
...
\medskip
Si l'on sait, dès la fin du XVIIe siècle intégrer les ED linéaires du premier et du second ordre 
à coefficients constants par des sommes d'exponentielles, il faut attendre 1760 pour que la 
théorie vienne à bout des ED linéaires à coefficients constants d'ordre quelconque. 
...
\ImageADroite{%
D'Alembert\index[aut]{d'Alembert (Jean le Rond), 1717-1783, Français} 
remarque que pour les ED non homogènes, l'adjonction d'une solution particulière 
...
C'est une ED très simple, mais qui modélise le problème dit de dynamique de population.\index{ED-EDP!dynamique de population}
...
il est nécessaire, pour déterminer complètement la solution d'une ED (ou d'une EDP),
...
\medskip
\begin{histoire}%
Concernant les EDP, elles sont initialement résolues par des méthodes ad-hoc, le plus souvent 
géométriques. 
On n'écrit pas l'EDP. On ne s'étonnera donc pas que la première équation aux dérivées partielles 
...
\medskip
\ImageADroite{%
La théorie de l'intégration des EDP du premier ordre ne date que de 1734. 
L'idée d'Euler\index[aut]{Euler (Leonhard Paul), 1707-1783, Suisse} 
est de ramener ladite intégration à celle des ED ordinaires. 
...
On parle de \textcolorblue{condition de Dirichlet} imposée à une ED ou une EDP,
...
On parle de \textcolorblue{condition de Neumann} imposée à une ED ou une EDP,
...
ou \textcolorblue{condition d'impédance} imposée à une ED ou une EDP,
...
On parle de \textcolorblue{condition aux limites dynamique} imposée à une ED ou une EDP,
...
\begin{itemize}
   \item \textcolorblue{*} si $\beta^2-4\alpha\gamma>0$;
...
L'\textcolorblue{équation de la chaleur} est une EDP parabolique, introduite initialement en 1811 par Fourier 
...
résoud le problème, notamment à l'aide du concept d'entropie de 
...
est analyste, i.e. un spécialiste des EDP, un sujet que beaucoup de topologues avaient 
...
\medskip
Les équations de Navier-Stokes sont des EDP non linéaires qui décrivent le mouvement 
...
- \left( \zeta + \frac{\eta}{3} \right) \overrightarrow{\mathrm{grad}} (\dive u)
= \rho f - \nabla p
\end{equation}
où :
\begin{itemize}
...
Dans le cas où la fonction $u$ est définie sur $\Omega\subset\RR^n$, une EDP elliptique 
...
Les EDP elliptiques conduisent à la notion physique de  flux conservatif donné
...
énergie (la rotation du moteur fait bouger tout le véhicule), qu'à des effets ayant

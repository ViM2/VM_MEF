...
Pour les méthodes explicites de résolution des équations différentielles, on ira voir au chapitre~\ref{Ch-ED}.
...
\medskip
\section{Conditions aux limites}\label{Sec-CL}\index{condition aux limites}
...
$A: \Omega\rightarrow\RR^{n\times n}$ sont les coefficients de l'équation aux dérivées partielles linéaire.
\medskipvm
...
\begin{itemize}
  ...
  \item les problèmes paraboliques vont concerner les modèles instationnaires tels que la diffusion thermique (équation de la chaleur), chimique, neutronique, les fluide visqueux incompressibles (on parlera d'irréversibilité, de décroissance, de principe du maximum, de propagation à vitesse infinie); 
...
\begin{histoire}%
\ifVersionDuDocEstVincent\fontfamily{cmr}\selectfont\fi
En dépit des railleries de Hugo\index[aut]{Hugo (Victor), 1802-1885, Français} et Stendhal\index[aut]{Beyle (Henri), dit Stendhal, 1783-1842, Français} Joseph Fourier\index[aut]{Fourier (Jean Baptiste Joseph), 1768-1830, Français} laissera son nom à la postérité à plus d'un titre:\\[-2.5mm]
\sbox{\MaBoiteAvecPhotos}{\setlength{\tabcolsep}{0pt}\scriptsize%
\begin{tabular}{cc}
...
f(x)=\dfrac{f(x^+)+f(x^-)}2
\end{equation}
D'ailleurs, un théorème de Dirichlet dit que pour une fonction~$f$ définie sur~$[-\pi,\pi]$ n'ayant qu'un nombre fini de discontinuités finies et ayant ailleurs une dérivée continue, son développement converge pour tout~$x\in[-\pi,\pi]$, que somme vérifie la condition de demi-somme en tout point de~$]-\pi,\pi[$, et qu'aux extrémités on a~$f(\pm\pi)=(f(-\pi^+)+f(\pi^-))/2$.
...
\end{itemize}
\medskipvm
L'équation de la chaleur, introduite initialement pour décrire la conduction thermique, permet de décrire le \textcolorblue{phénomène de diffusion}.
...
\medskip
\ImageADroite{%
En 1956, Louis Nirenberg\index[aut]{Nirenberg (Louis), 1925- , Américain} propose un nouveau problème à John Nash\index[aut]{Nash (John Forbes), 1928- , Américain} (qui est, selon Warren Ambrose,\index[aut]{Ambrose (Warren Arthur), 1914-1995, Américain} «aussi pressé que X, aussi exaspérant que Y, quels que soient X et Y»): la continuité des solutions de l'équation de la chaleur dans les milieux discontinus, i.e. lorsque le coefficient de conductivité est quelconque, et peut même varier brutalement d'un point à l'autre et que l'on ne dispose seulement que des bornes inférieure et supérieure sur la conductivité.\\ \indent
...
\begin{remarque}[L'effet régularisant]\colorgris
Considérons l'équation de la chaleur~(\ref{Eq-Chaleur}) avec condition initiale~$T_0(x)$ et~$f=0$, que nous réécrivons:
\[\left\{
\begin{array}{ll}
\dfrac{\partial T}{\partial t} -\Delta T = 0 & \text{ dans } \intoo0\infty\times\RR^N\\
T(x,0) = T_0(x) & \text{ dans } \RR^N
\end{array}\right.
\]
Nous nous proposons de montrer que la solution~$T$ appartient en fait à~$C^\infty(\RR^N\times\RR_*^+)$.

\medskip
Soit~$\hat{T}(\xi,t)=\hat{T}_0(\xi)e^{-|\xi|^2t}$ la transformée de Fourier de~$T$.
Pour tout multi-indice~$\alpha$ et pour tout~$t>0$, la fonction~$(\xi,t) \mapsto |\xi|^\alpha\hat{T}(\xi,t)$ est continue en temps et à valeurs dans~$L^2(\RR^N)$.
Par transformation de Fourier inverse, $\partial^\alpha T$ est donc un élément de~$C(\RR^+_*,L^2(\RR^N))$, i.e. pour tout entier~$m$, $T$ est dans~$C(\RR^+_*,H^m(\RR^N))$.
Les injections de Sobolev nous permettent donc de dire que~$T(t)$ appartient à~$C(\RR^+_*,C^\infty(\RR^N))$.
La même analyse tient pour~$\partial^n T/\partial t^n$, et donc, on en déduit que~$T\in C^\infty(\RR^+_*,C^\infty(\RR^N))=C^\infty(R^N\times\RR_*^+)$.
\end{remarque}
...
Deux mots en aparté sur le flot de Ricci dont on entend beaucoup parler en ce moment...\\ \indent 
...
\medskip
D'un point de vue de la formulation de ce problème sous forme d'équation aux dérivées partielles, tout à été dit précédemment. Nous verrons plus loin, aux chapitres~\ref{Ch-ondes} et surtout~\ref{Ch-Acous}, comment prendre en compte tous ces aspects de manière plus appliquée.
...
\medskip
Les multiplicateurs de Lagrange seront également repris au chapitre~\ref{Ch-Optim} sur l'optimisation, où la notion de Lagrangien, abordée au paragraphe~\ref{Sec-MultLag}, sera beaucoup plus détaillée.

...
\medskipvm
Considérons une plaque avec un trou elliptique, de grand axe~$2a$ et petit axe~$2b$, dont la largeur~$\ell$ est très supérieure aux dimensions du trou (i.e.~$\ell\gg 2a$) et soumise à une contrainte nominale constante $\sigma$, comme illustré sur la figure~\ref{fig:fissure}. La contrainte en pointe de fissure (i.e. au point A) est:
...
GFRP \textcolorgris{(CFRP = Carbon Fiber Reinforced Polymer = polymère à renfort fibre de carbone; GFRP = Glass Fiber Reinforced Polymer = polymère à renfort fibre de verre)} peut être décrite (entre~$10^3$ et~$10^6$ cycles) par une équation de la forme: \begin{equation}  \dfrac{\sigma_a}{\sigma_u}=1-b\log N \end{equation} 

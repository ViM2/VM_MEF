...
%une variable que l'on appelera plus tard le taux de restitution d'énergie.
%
...
\begin{itemize}
   ...
   \item \textcolorblue{Approche locale}: qui veut \emph{modéliser finement le comportement réel}, 
	mais se confronteà la \textcolorred{singularité des contraintes en fond de fissure}.
\end{itemize}
...
\begin{itemize}
   \item \textcolorblue{Fissure droite} sollicitée en mode I :
	...
	pertinentes.
   \item \textcolorblue{Fissure courbe} - méthodes de remaillage :
	voir problème de maillage
   \item \textcolorblue{Fissure courbe} - éléments d'interface :
	le trajet de la fissure est imposé par la discrétisation et il faut choisir
	la loi de décohésion à l'interface.
   \item Il reste l'astuce de l'\textcolorblue{élément ayant un nœud supplémentaire} 
   ...
   \item Les méthodes \textcolorblue{sans maillage}: elles permettent de s'affranchir
...
Inglis\index[aut]{Inglis} (1913, cité au 8ème symposium  de mécanique des roches en 1966) 
...
\medskip
\ifVersionDuDocEstVincent\begin{tabular}{lp{125mm}}\else\begin{tabular}{lp{105mm}}\fi
\raisebox{-43mm}{\includegraphics[height=45mm]{fissure.eps}} &
Inglis,\index[aut]{Inglis} donc, en 1913, a montré l'effet de \textcolorblue{concentration de contraintes}.

\medskip
Considérons une plaque avec un trou elliptique (de grand axe 2a et petit axe 2b) dont la largeur $l$ est
très supérieure aux dimensions du trou (i.e. $\gg 2a$), et soumise à une contrainte nominale constante
$\sigma$.
...
Le \textcolorblue{facteur de concentration de contraintes $k_f$} est défini comme:
$k_f = \sigma_A/\sigma $

\medskip
\begin{itemize}
   \item Si $a=b$, alors $k_f=3$;
   \item Si $a\rightarrow\infty$, Ingliss propose $\sigma_A=\sigma \left( 1+2\sqrt{\frac{a}{\rho}}\right)$, 
	où $\rho=b^2/A$ est le rayon en fond (ou pointe) de fissure;
...
\indent\indent 1) La contrainte nécessaire pour rompre un verre courant est d'environ 100 MPa, et;\\
\indent\indent 2) La contrainte théorique nécessaire à la rupture de liaisons atomiques est d'environ 10 000 MPa.
...
\medskip
\ifVersionDuDocEstVincent\begin{tabular}{p{120mm}l}\else\begin{tabular}{p{105mm}l}\fi
En 1920, Griffith\index[aut]{Griffith (Alan Arnold), 1893-1963, Anglais} considère le cas d'une plaque 
d'épaisseur $B$ contenant une fissure de longueur $2a$ et dont la largeur $l$ est très supérieure 
aux dimensions de la fissure (i.e. $l\gg 2a$) et à son épaisseur
(i.e. $l\gg B$), soumise à une contrainte nominale constante $\sigma$.
...
\begin{equation} \frac{dE}{dA} = \dfrac{d\Pi}{dA}+\frac{dW_s}{dA} = 0 \end{equation}
où $A=2aB$ est l'aire de la fissure, $dA$ l'accroissement de fissure, $E$ l'énergie
totale, $\Pi$ l'énergie potentielle et $W_s$ l'énergie nécessaire pour la progression du
défaut.

&\raisebox{-43mm}{\includegraphics[height=45mm]{fissure2.eps}}
\end{tabular}
...
$-\frac{d\Pi}{dA}=\frac{dW_s}{dA}$ et donc $\frac{\pi a\sigma^2}{E}=2\gamma_s$.
...
\begin{equation} G = \dfrac{d\Pi}{dA} \end{equation}
\textcolorgris{En négligeant l'énergie cinétique, la puissance disponible pour ouvrir une fissure 
de surface $A$ est égale à la variation dénergie potentielle totale, résultat de la variation de l'énergie 
...
\begin{equation} G_c = \dfrac{dW_s}{dA}=2\gamma_s \end{equation}
où $\gamma_s$ est l'énergie de progression du défaut par unité de surface.
$G_c$ est la mesure de ténacitéé à la rupture du matériau.
...
\medskip\noindent
\ifVersionDuDocEstVincent\begin{tabular}{lp{139mm}}\else\begin{tabular}{lp{118mm}}\fi
\raisebox{-38mm}{\includegraphics[height=40mm]{fissIrwin.eps}} &
...
\medskip\noindent
\ifVersionDuDocEstVincent\begin{tabular}{p{102mm}l}\else\begin{tabular}{p{90mm}l}\fi
En posant:
...
fissure, on a:
\begin{equation} \lim_{r\rightarrow0} \sigma_{ij}^{(I)} = \frac{K_I}{\sqrt{2\pi r}} f_{ij}^{(I)}(\theta) \end{equation}
\begin{equation} \lim_{r\rightarrow0} \sigma_{ij}^{(II)} = \frac{K_II}{\sqrt{2\pi r}} f_{ij}^{(II)}(\theta) \end{equation}
\begin{equation} \lim_{r\rightarrow0} \sigma_{ij}^{(III)} = \frac{K_III}{\sqrt{2\pi r}} f_{ij}^{(III)}(\theta) \end{equation}
&
\raisebox{-45mm}{\includegraphics[height=40mm]{fisspol.eps}}
\end{tabular}

\noindent trois modes de chargements pouvant s'appliquer à une fissure. Ces trois modes sont donnés à la
\fig{Fig-modesR}.
\begin{figure}[htb]
\centerline{\includegraphics[height=55mm]{modesR.eps}}
...
Par les contraintes: par exemple $K_I$ sera pris égale à la valeur obtenue (post-traitée)
...
Par les déplacements: $K_I$ est obtenu par $\nu \sqrt{2\pi/r}$ en fond de fissure (obtenu
...
\begin{equation} G_I = \dfrac{K_I^2}E \qquad \text{ et en déformations plane: }
G_I = (1-\nu^2)\dfrac{K_I^2}E \end{equation}
\begin{equation} G_{II} = \dfrac{K_{II}^2}E \qquad \text{ et en déformations plane: }
G_{II} = (1-\nu^2)\dfrac{K_{II}^2}E \end{equation}
\begin{equation} G_{III} = (1+\nu)\dfrac{K_{III}^2}E \end{equation}
...
\medskip\noindent
\ifVersionDuDocEstVincent\begin{tabular}{lp{135mm}}\else\begin{tabular}{lp{114mm}}\fi
\raisebox{-38mm}{\includegraphics[height=40mm]{j1.eps}} &
...
La densité d'énergie interne élastique est $\omega=\dint_0^{\varepsilon_{kl}} \sigma_{ij}d\varepsilon_{ij}$. 

On travaille en l'absence de forces de volumes: $\sigma_{ij,i}=0$

et sous hypothèse de petites déformations: $\varepsilon_{ij}=(u_{i,j}+u_{j,i})/2$
\end{tabular}

Alors les intégrales curvilignes suivantes:
$ Q_j =\dint_\Gamma \left(\omega n_j- T_k u_{k,j}\right)d\Gamma$ pour $j, k=1,2,3$
peuvent s'écrire:
$ Q_j =\dint_\Gamma \left(\omega n_j- \sigma_{lk}n_l u_{k,j}\right)d\Gamma 
=\int_\Gamma \left(\omega \delta_{jl}- \sigma_{lk} u_{k,j}\right)n_l d\Gamma $

Le théorème de divergence puis quelques manipulations de l'opérande permettent d'arriver au résultat
suivant:
\textcolorblue{$ Q_j = 0$}.
De là, on en tire que la densité dénergie interne élastique
$\omega=\dint_0^{\varepsilon_{kl}} \sigma_{ij}d\varepsilon_{ij} $
...
$ \Omega = \dint_0^{\sigma_{kl}} \varepsilon_{ij}d\sigma_{ij} $
...
On définit \textcolorblue{l'intégrale $J$} comme:
\begin{equation} J = Q_1 =\dint_\Gamma \left(\omega n_1- T_k u_{k,1}\right)d\Gamma \end{equation}

D'après ce qui précède, \textcolorblue{$J=0$}, et donc
$J_1=\dint_{\Gamma_1} ... = J_2=\dint_{\Gamma_2} ...$.

\medskip\noindent
\ifVersionDuDocEstVincent\begin{tabular}{p{98mm}l}\else\begin{tabular}{p{80mm}l}\fi
\indent Considérons maintenant une entaille (fissure) dans une pièce.
...
&\hspace{5mm} 
\raisebox{-35mm}{\includegraphics[height=37mm]{j3.eps}}
\end{tabular}
...
du fond de fissure, et qu'au delà, jusquà un rayon $D$ la solution singulière est
...
\medskip
%\ifVersionDuDocEstVincent\begin{cutout}{0}{0.45\linewidth}{0pt}{8}\else\begin{cutout}{0}{0.39\linewidth}{0pt}{8}\fi
\ifVersionDuDocEstVincent\begin{tabular}{lp{107mm}}\else\noindent\begin{tabular}{lp{91mm}}\fi
\raisebox{-48mm}{\includegraphics[height=50mm]{plast.eps}}&
...
La zone plastique est donnée par: $r_p(0)=\frac1{2\pi}\left(\frac{K_I}{\sigma_Y}\right)^2$ ($\sigma_Y$
étant la limite élastique).

Et donc, si $0<r<r_1$, $\sigma_2=\sigma_Y$ et lorsque $r>r_1$, $\sigma_2=\frac{K_I}{\sqrt{2\pi r}}$.
Mézalor, comme $\sigma_2=\sigma_Y$ est constant pour $r < r_1$, il y a violation d'équilibre suivant l'axe $y$.
La réduction de $\sigma_2$ est compensée par $\sigma_1$ d'où une augmentation de $r_1$.

On obtient une zone plastique corrigée 2 fois plus grande:
\begin{equation} c = \frac1\pi\left(\frac{K_I}{\sigma_Y}\right)^2 \qquad \text{ en contraintes planes}\end{equation}
\begin{equation} c = \frac1{3\pi}\left(\frac{K_I}{\sigma_Y}\right)^2 \qquad \text{ en déformations planes}\end{equation}
et à l'extrémité de la fissure, l'ouverture est $\delta$:
\begin{equation} \delta = \frac4{\pi E}\frac{K_I^2}{\sigma_Y} \qquad \text{ en contraintes planes}\end{equation}
\begin{equation} \delta = \frac{4(1-\nu^2)}{3\pi E}\frac{K_I^2}{\sigma_Y} \qquad \text{ en déformations planes}\end{equation}
...
\medskip
\subsubsection{Modèle de Dugdale}\index[aut]{Dugdale}
...
Cette approche, introduite en 1961 par Wells\index[aut]{Wells} et Cottrell, postule qu'il y a initiation de fissures 
...
puis les contraintes sont obtenues via la loi de comportement: $\sigma=H\varepsilon$.

Si le matériau est élastoplastique, les contraintes seront calculées de façon
incrémentale: $\Delta\sigma=H(\sigma,\varepsilon)\Delta\varepsilon$.

\medskip
\subsubsection{Les éléments finis singuliers}\index{Elément singulier}

Certains éléments ou configurations entrainent des \textcolorblue{singularités des déformations}.
...
les nœuds des milieux de cotês de $1/4$.
...
\medskip\noindent
\ifVersionDuDocEstVincent\begin{tabular}{p{122mm}l}\else\begin{tabular}{p{100mm}l}\fi
\indent Supposons maintenant que les nœuds 5 et 8 soient déplacés de 1/4 vers le nœud 1 (qui est l'origine 
du repère).
...
&\hspace{5mm}
\raisebox{-43mm}{\includegraphics[height=45mm]{EltSing.eps}}
\end{tabular}
...
Le terme du jacobien est: $\dfrac{\partial x}{\partial\xi}=\dfrac{L}2(1+\xi)=\sqrt{xL}$
...
Il est possible d'éffectuer un \textcolorblue{post-traitement des contraintes} en créant 
...
$\Pi = \frac12\LL{q}\MM{K}\VV{q}-\LL{q}\VV{F}$. 
...
\begin{itemize}
   ...
   \item  Distorsion des éléments en grandes déformations (ou alors regénération
...
\end{itemize}
Une idée \og simple\fg{} est d'éliminer les éléments...
...
ces dernières décénies, on se heurte à  l'absence de méthodes numériques suffisamment 
...
Tant que les fissures restent suffisament petites, il n'y à a pas de risque de rupture.
...
$K_{Ic}$ est la valeur critique qui correspond à une rupture instantannée par dépassement de la
...
\begin{itemize}
   ...
   \item \textcolorblue{\em théories de dégradation de résistance résiduelle:}
        ...
        de Weibull\footnote{%
En théorie des probabilités, la loi de Weibull,\index{Loi de Weibull}\index[aut]{Weibull (Ernst Hjalmar Waloddi), 1887-1979, Suédois} 
...
\begin{itemize}
	\item Un taux de panne décroissant relève d'une \og mortalité infantile\fg{}. Ainsi, les 
	...
	\item Un taux de panne croissant suggère une \og usure ou un problème de fiabilité\fg{} : 
...
Les fabricants et distributeurs ont tout intérêt à bien maitriser ces informations par type de produits 
afin d'adapter :
   1. les durées de garantie (gratuites ou payantes)
   2. le planning d'entretien
}% 
        de la résistance des composites.
         La rupture par fatigue survient lorsque la résistance résiduelle
         du composite est atteinte par la contrainte appliquée.
...
\begin{itemize}
	...
	\item \textcolorblue{amplitude}: une sollicitation de grande amplitude suivie d'une sollicitation de faible amplitude
		...
		inversé.
	\item \textcolorblue{$R=-1$} (rapport entre charge positive et négative dans l cyclage) : ce cas est très
	...
	\item \textcolorblue{la forme du signal} a une inflence qui serait à considérer.

...
On rappelle que les nœuds sont déterminés comme les~$n$ racines du~$n$ème polynôme orthogonal associé à la formule de quadrature. 
 \emph{Les méthodes de quadrature de Gauß intègrent exactement un polynôme de degré~$2n-1$ avec~$n$ points.}\index[aut]{Gauß (Johann Carl Friedrich), 1777-1855, Allemand}\index{méthode de quadrature de Gauß} 
 
%%%%----------------------------------------------------------
\subsection
%%%%----------------------------------------------------------
*{Changement d'intervalle d'intégration} Si on intègre sur~$(a,b)$ au lieu de~$(-1,1)$, alors on fait un changement de variable. Finalement, on obtient l'approximation: 
\begin{equation}
 \frac{b-a}{2} \sum_{i=1}^n \varpi_i f\left(\frac{b-a}{2}x_i + \frac{a+b}{2}\right) 
\end{equation}
\begin{remarque} \mbox{}
\begin{itemize} 
\item pour la méthode des éléments finis, l'intégration se déroule sur l'élément de référence, donc on n'a pas besoin de faire ce changement. Il est fait par la transformation affine entre l'élément considéré et l'élément de référence;
\item le nombre de points de Gauß\index[aut]{Gauß (Johann Carl Friedrich), 1777-1855, Allemand} ainsi que leurs positions sur l'élément sont donnés dans les documentations des logiciels. 
\end{itemize}
\end{remarque}
\begin{figure}[ht] 
\centering

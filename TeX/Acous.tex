...
Face à une problème vibroacoustique, on est amené à considérer les questions suivantes:
\begin{itemize}
...
Pour des longueurs d'ondes inférieures à la moitié des dimensions de la salle: on est dans le cas de réflexions multiples, et on peut faire une équivalence lumineuse et utiliser les méthodes de tir de rayon de type «raytracing».
...
Dans le cas harmonique, l'équation précédente devient l'équation d'Helmholtz~(\ref{Eq-Helm})\index[aut]{Helmholtz (Hermann Ludwig Ferdinand von -), 1821-1894, Allemand} avec termes sources:
\begin{equation}
-\dfrac{\omega^2p}{\rho c^2}+\nabla.\left(-\dfrac{1}{\rho}(\nabla p-q)\right)=Q
\end{equation}
Si l'on souhaite introduire le d'amortissement dans l'équation, alors les quantités~$c$ et~$\rho$ peuvent être complexes (au sens appartiennent à~$\CC$). Par exemple, pour un modèle d'amortissement de Rayleigh, $c$ est une combinaison linéaire de la masse et de la raideur.


\medskip
Pour réaliser notre modèle de calcul vibroacoustique par éléments finis, nous devrons disposer de sources et de conditions aux limites:
\begin{itemize}
...
Un silencieux d'automobile peut n'être défini que par le volume d'air qu'il enferme. On appliquera aux parois l'impédance correspondant aux parois: soit réflexion quasiment pure avec un peu de perte, soit présence d'un absorbant en parois. La surface correspondant à l'entrée de l'onde dans le silencieux sera modélisée par la surface soumise à une onde plane. La surface correspondant à la sortie du silencieux sera la surface où les informations (de type onde plane) seront recueillies puis analysées.

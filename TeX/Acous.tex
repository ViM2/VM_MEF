...
Le théorème de Kirchhoff dit que le champ de pression, en régime fréquentiel, en un point d'observation~$x$ donné est entièrement déterminé par les valeurs de la pression et de sa dérivée normale sur les bords du domaine et la donnée de la source volumique~$q(x_0)$. Il stipule que le champ de pression au point~$x$ est donné par la relation:
\begin{equation}
p(x)=\dint_{\Gamma=\partial\Omega} p(x_0)\dfrac{\partial G(x,x_0)}{\partial n_0} -G(x,x_0)\dfrac{\partial p(x_0)}{\partial n_0}\,\dd\Gamma + \dint_\Omega q(x_0)G(x,x_0)\, \dd\Omega
\end{equation}
\end{theoreme}

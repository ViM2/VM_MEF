...
\begin{flushright}
\textcolorgris{\reflectbox{Yaka essayer... ici pas de différence car~$\Im(p)=0$, et donc on peut rester dans~$\RR$.}}
\end{flushright}
...
\medskip
\subsection{Interaction fluide-structure forte: un tablier léger}
%
...
Nous proposons donc un exemple dans lequel il y a interaction fluide-structure forte entre un domaine fluide (air) et un domaine modélisé par l'élasticité linéaire isotrope. 

\medskip
Commençons par le solide. Sa formulation variationnelle est donnée à l'équation~(\ref{Eq-StatLinIso}) rappelée ici:
\begin{equation*}

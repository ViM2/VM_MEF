...
\end{equation}
Les modes~$(i,0,0)$, $(0,j,0)$ et~$(0,0,k)$ sont des modes longitudinaux dans la directions de~$L$, $l$ et~$h$ respectivement. Les autres modes sont des modes transverses.
...
\medskip
\subsection{Quelques exemples de modèle éléments finis avec ou sans couplage}

Comme nous l'avons déjà évoqué, un certain nombre de calculs ne nécessitent pas de recourir à un couplage fluide/structure. En voici quelques exemples concrets:
\begin{itemize}
   \item Cas solidien/solidien: il s'agit par exemple d'étudier la propagation d'ondes de compression ou de cisaillement au sein d'une structure. Seule la structure sera modélisée.
   \item Cas aérien/aérien: l'acoustique des salles d'une manière générale peut se passer de couplage. Seul le volume de la salle est à discrétiser. Les zones d'absorption situées en parois seront modélisées par leur impédance, ainsi que les pertes par transmission. Dans le cas d'un locuteur, la source sera ponctuelle.
   \item Cas aérien/aérien: un silencieux d'automobile peut n'être défini que par le volume d'air qu'il enferme. On appliquera aux parois l'impédance correspondant aux parois: soit réflexion quasiment pure avec un peu de perte, soit présence d'un absorbant en parois. La surface correspondant à l'entrée de l'onde dans le silencieux sera modélisée par la surface soumise à une onde plane. La surface correspondant à la sortie du silencieux sera la surface où les informations (de type onde plane) seront recueillies puis analysées.
\end{itemize}
...
Des conditions de couplage devront être mise en place entre ces deux domaines. Certains logiciels les prennent quasiment tout seul en compte, pour les autres il faudra le faire manuellement.

\textcolorred{A COMPLETER EXPLICITEMENT}

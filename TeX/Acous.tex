...
solve sound(u,v)= int2d(Th)(u*v*k - dx(u)*dx(v) - dy(u)*dy(v))
 -int1d(Th,a4)(g*v) + on(a1,u=0) + on(a2,u=0)+ on(a3,u=0);
plot (u, wait=1, ps="AcouDiff.eps");
\end{Verbatim}
\colorblack\normalsize
Le résultat est donné à la figure~\ref{Fig-AcouDiff} pour plusieurs valeurs de la constante~\verb|k|.
\begin{figure}[h!]
\centering
\ifVersionDuDocEstVincent
   \includegraphics[width=57mm]{AcouDiff1.eps}\hfill
   \includegraphics[width=57mm]{AcouDiff3.eps}\hfill
   \includegraphics[width=57mm]{AcouDiff100.eps}
\else
   \includegraphics[width=50mm]{AcouDiff1.eps}\hfill
   \includegraphics[width=50mm]{AcouDiff3.eps}\hfill
   \includegraphics[width=50mm]{AcouDiff100.eps}
\fi
\caption{Acoustique au coin de la rue pour $k=1$, $3$ et $100$}\label{Fig-AcouDiff}
\end{figure}
Nous discuterons du bien fondé de la modélisation, et en particulier des conditions aux limites introduites.

...
par exemple, \og la plus petite période du système\fg{} au chapitre précédent... mais cela a été fait
à dessein.
...
exemples de ce phénomène dans la nature:%\footnote{%
%\begin{tabular}{cc}
%\includegraphics[height=32mm]{Huygens}&
%\includegraphics[height=32mm]{Cantor}\\
%Christian Huygens&
...
(issues par exemple de la cloture algébrique de $\CC$ démontrée par Gau\ss).\index[aut]{Gau\ss{} (Johann Carl Friedrich), 1777-1855, Allemand}
...
\medskip
\section{Notions de valeur, vecteur, mode, fréquence propres}\index{Valeur propre}\index{Vecteur propre}\index{Mode propre}\index{Fréquence propre}

\medskip
Étant donné une matrice carrée $A$ d'ordre $n$ \textcolorgris{(à coefficients dans un anneau commutatif)}, 
...
\medskip
Si $\lambda$ est une \textcolorblue{valeur propre}\index{Valeur propre} de $A$, alors il existe un 
\textcolorblue{vecteur propre}\index{Vecteur propre} $V$ non nul tel que $AV = \lambda V$, i.e. 
...
les valeurs propres\index{Valeur propre} de $A$ sont exactement les racines du polynôme $p_A(X)$. 
...
On trouve également les $n$ vecteurs $\VV{\overline{q_i}}$ appelés \textcolorblue{modes propres 
...
La \textcolorblue{pulsation propre} du système $\omega_0$, sa \textcolorblue{fréquence propre} $f_0$
et sa \textcolorblue{période propre} $T_0$ sont définies et reliées par les relations:
\begin{equation}
...
$\LL{\overline{q_i}}$), on obtient un \textcolorblue{ensemble d'équations scalaires indépendantes}:
\begin{equation} m_i\ddot{y_i}+c_i\dot{u_i}+k_iy_i+F_i = 0 \end{equation}
dont les paramètres sont, grâce \ a l'orthogonalité des modes:\index{Propriété d'orthogonalité}
\begin{equation}\left\{
\begin{array}{rcl}
...
d'ortogonalité de $\MM{C}$}: \begin{equation}\LL{\overline{q_i}}\MM{C}\VV{\overline{q_i}}=0\end{equation}
...
\begin{equation} \omega_i^2\MM{M}\VV{\overline{q_i}} =\MM{K}\VV{\overline{q_i}} \end{equation}
et par suite:
...
\begin{equation} y_i = \dint_0^t F_i e^{-c'_i\omega_i(t-\tau)}\sin \omega_i (t-\tau) d\tau \end{equation}
...
Or, physiquement, on s'aperçoit que les modes propres (et surtout les permiers modes propres) réalisent cet optimal.}

\medskip
Il existe deux principaux types de méthodes:
\begin{itemize}
   \item les méthodes à interfaces libres (Craig...);\index[aut]{Craig (R. R. Jr), ?, Américain}
   \item et les méthodes à interfaces fixes (Craig-Bampton).\index[aut]{Bampton (M. C. C.), ?, Américain}\index[aut]{Craig (R. R. Jr), ?, Américain}
\end{itemize}
...
\medskip
\subsubsection{Sous-structuration: méthode de Craig et Bampton}\index[aut]{Bampton (M. C. C.), ?, Américain}\index[aut]{Craig (R. R. Jr), ?, Américain}

Considérons une structure comportant $n$ ddl et ayant une matrice masse $\MM{M}$ et une matrice rigidité $\MM{K}$. 
L'utilisation de la méthode de Craig-Bampton\index[aut]{Bampton (M. C. C.), ?, Américain}\index[aut]{Craig (R. R. Jr), ?, Américain} 
...
de la \og dynamique lente\fg{}, sans rien enlever en généralité aux méthodes.}
...
La solution temporelle est ensuite reconstruite par tranformée de Fourier\index[aut]{Fourier (Jean Baptiste Joseph), 1768-1830, Français}\index{Transformée de Fourier} 
inverse.
...
La modélisation est maîtrisée : \textcolorblue{EF} sur base modale, complété si besoin de modes
statiques.
...
Si la structure est divisible en sous-structures homogènes, on peut utiliser la \textcolorblue{Thérorie
...
Chevreuil et al.\index[aut]{Chevreuil (Mathilde), ?- , Française}
...
\textcolorblue{L'approche fréquentielle est également plus adpatée dans les situations pour 
...
La dispersion numérique est moindre avec les EF stabilisés :  tels que les \textcolorblue{Galerkin 
...
\bigskip
Une \og deuxième \fg{} approche consiste à utiliser des \textcolorblue{éléments enrichis}\index{Elément enrichi}
...
\medskip 
Les \textcolorblue{EF hiérarchiques,\index{EF hiérarchique} issus des méthodes p}\index{Méthode p} 
...
Les \textcolorblue{EF multiéchelles} (voir paragraphe \ref{Sec-ssstruc}) où la solution est 
...
Lorsque cette méthode est applicable \textcolorblue{elle donne de bon résultats aussi bien en BF, en MF et en HF}.
...
\bigskip
La \textcolorblue{SEA\index{SEA} (Statistical Energy Analysis) est la méthode de référence pour les HF}.
...
La SEA\index{SEA} est parfaitement adaptée aux HF, mais trop globale et trop imprécise pour
...
On utilisera également la règle classique d'\textcolorblue{au moins 10 éléments linéaires
par longueur d'onde pour le maillage}.

La FFT\index{Transformée de Fourier}\index{FFT} requiert par ailleurs que le chargement soit périodique. 
...
pour utiliser la FFT\index{Transformée de Fourier}\index{FFT} et son efficacité, et $N$ soit être tel que
...
donc la FFT\index{Transformée de Fourier}\index{FFT} coûteuse.
Dans ces cas, des méthodes ont été développées : les fonctions de Green;\index[aut]{Green (George), 1793-1841, Anglais}

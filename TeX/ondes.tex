...
%%%%%%%%%%%%%%%%%%
\medskipvm
En mathématiques, le concept de vecteur propre\index{vecteur propre|see{mode propre}}\index{mode propre} est une notion portant sur une
...
Ce rapport de dilatation est appelé valeur propre;\index{valeur propre|see{fréquence propre}}\index{fréquence propre} les vecteurs auxquels il s'applique s'appellent
...
des structures algébriques nécessaires pour vraiment pouvoir parler des valeurs propres\index{valeur propre|see{fréquence propre}}\index{fréquence propre}
(issues par exemple de la cloture algébrique de~$\CC$ démontrée par Gauß).\index[aut]{Gauß (Johann Carl Friedrich), 1777-1855, Allemand}
...
une base de vecteurs propres,\index{vecteur propre|see{mode propre}}\index{mode propre} bien que le concept n'ait pas encore été
...
que l'on appelle maintenant les valeurs propres\index{valeur propre|see{fréquence propre}}\index{fréquence propre} associées à l'endomorphisme d'une équation différentielle linéaire.
...
Cayley)\index[aut]{Cayley (Arthur), 1821-1895, Anglais} et de valeurs propres\index{valeur propre|see{fréquence propre}}\index{fréquence propre}
...
conscience de la profondeur de la notion de valeur propre.\index{valeur propre|see{fréquence propre}}\index{fréquence propre}
...
%%%%%%%%%%%%%%%%%%%%%%
\medskipvm
Si~$\lambda$ est une \textcolorblue{valeur propre}\index{valeur propre|see{fréquence propre}}\index{fréquence propre} de~$\MM{A}$, alors il existe un
\textcolorblue{vecteur propre}\index{vecteur propre|see{mode propre}}\index{mode propre}~$\VV{V}$ non nul tel que~$\MM{A}\VV{V} = \lambda \VV{V}$, i.e.
...
les valeurs propres\index{valeur propre|see{fréquence propre}}\index{fréquence propre} de~$\MM{A}$ sont exactement les racines du polynôme~$p_A(X)$.
...
%%%%%%%%%%%%%%%%%%%%%%
\medskipvm
On pourrait considérer que le chapitre~\ref{Ch-temps} répond à ces trois
...
avoir résolu le problèmes des valeurs propres.\index{valeur propre|see{fréquence propre}}\index{fréquence propre}
...
par rapport à sa valeur critique}), on montre que les équations scalaires se mettent sous forme d'une équation différentielle
du second ordre:
...
nécessiterait la détermination de l'ensemble des valeurs et modes propres,\index{valeur propre|see{fréquence propre}}\index{fréquence propre}\index{mode propre}
...
(à l'aide de la FFT),\index{transformée de Fourier} ce qui conduit à résoudre un problème de vibrations forcées sur une très large bande
...
Variationnelle des Rayons Complexes (TVRC)}\index{Théorie Variationnelle des Rayons Complexes} introduite
...
la FFT).\index{transformée de Fourier}
...
Cette approche peut être couplée à la \textcolorblue{théorie des structures floues}\index{théorie des structures floues}
...
\bigskip
Une <<~deuxième~>> approche consiste à utiliser des \textcolorblue{éléments enrichis}\index{élément fini!enrichi}
...
\medskip
Les \textcolorblue{éléments finis hiérarchiques,\index{élément fini!hiérarchique} issus des méthodes~$p$}\index{méthode~$p$}
...
La \textcolorblue{Méthode de partition de l'unité (PUM)}\index{partition de l'unité} (voir paragraphe~\ref{Sec-partition}) utilise un recouvrement
...
\bigskip
La \textcolorblue{BEM}\index{élément fini!frontière}
...
Dans la EFGM (Element Free Galerkin Method ou méthode de Galerkine sans maillage),\index{méthode de Galerkine!sans maillage}
...
formant une partition de l'unité,\index{partition de l'unité} qui peuvent être polynomiales
ou sinusoïdales.
...
\bigskip
Les \textcolorblue{méthodes des éléments discontinus}:\index{méthode des éléments discontinus}

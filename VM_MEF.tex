...
\hypersetup{hidelinks,backref=true, pagebackref=true,hyperindex=true,colorlinks=false,breaklinks=true,urlcolor= cyan,bookmarks=true,bookmarksopen=false,pdftitle={Méthode des Éléments Finis},pdfauthor={Vincent Manet}, pdfsubject={Aspects mathématiques et illustrations pour l'ingénieur}}
\usepackage{listings}
\usepackage{xcolor}
\usepackage{./tex/freefempp}
\usepackage{./tex/castem}
\usepackage{./tex/ansys}
\usepackage{times}% normalement obsolete et remplacé par \usepackage{mathptmx}
\gdef\ttdefault{cmtt}%
\usepackage[amsbb,compatiblegreek,subscriptcorrection,nofontinfo,mtpscr]{mtpro2}
%
\ifVersionDuDocEstVincent
%   \usepackage{amssymb,bm}% pas compatible avec fontes MLg
...
\thispagestyle{empty}\mbox{}%
   \pagecolor[rgb]{0.9,0.9,0.9}% gris clair
%\pagecolor[rgb]{0.8,0.8,0.8}% correspond au gris de VM_MEF v1 à 7
%\afterpage{\nopagecolor}% nécessite le package afterpage
   \setlength{\abovedisplayskip}{6pt plus 2pt minus 1pt}
   \setlength{\belowdisplayskip}{\abovedisplayskip}
%   ~\\[+15cm]\centerline{Version 4 du \today}
   ~\\[+1.2cm]
   \centerline{\scalefont{3.3}\selectfont\textbf{Vincent Manet}}\\[+70mm]
   \centerline{\scalefont{4.5}\color{ocre}\fontfamily{phv}\selectfont\textbf{Méthode des}}\\[+3.5mm]
   \centerline{\scalefont{4.5}\color{ocre}\fontfamily{phv}\selectfont\textbf{éléments finis}}\\[+90mm]
   \begin{flushright} \scalefont{2}\selectfont\emph{Vulgarisation des aspects mathématiques\\et illustration de la méthode} \end{flushright}
% Verso de la page de titre
\newpage\nopagecolor\mbox{}\thispagestyle{empty}%
\vspace*{2cm}
\vfill\normalsize\selectfont
\noindent Vincent Manet --- 2012--2018 (Ceci est la version \ifVersionDuDocEstVincent \textcolorblue{«cours»} \else \textcolorblue{«livre»} \fi de ce document)\\
...
Nous remercions Mathias Legrand\index[aut]{Legrand (Mathias), ?- , Français} pour ses conseils avisés et sa relecture pertinente.
...
Cela nous permettra d'écrire~$\LL{x}=\LL{x_1, ..., x_n}$ pour définir le vecteur~$x$, plutôt que d'écrire~$\VV{x}=\VV*{x_1\\ \vdots\\x_n}$, ce qui rend le texte beaucoup plus lisible. \textcolorgris{Par ailleurs, on trouve souvent des écritures du type $x=(x_1, ..., x_n)$, qui se comprennent, mais que nous n'aimons pas: un vecteur, c'est une colonne; une ligne, c'est un vecteur transposé!}
